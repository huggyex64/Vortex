...
namespace Vortex.Generators;

[Generator]
public class EventDomainGenerator : IIncrementalGenerator
{
    private const string EventDomainAttrFqn = "Vortex.EventDomainAttribute";
    private const string EventArgsAttrFqn = "Vortex.EventArgsAttribute";
    private const string EventKeyFqn = "global::Vortex.EventKey";
    private const string UnitFqn = "global::Vortex.Unit";
    ...
    private static EventDomainInfo? GetDomainInfo(GeneratorAttributeSyntaxContext ctx, CancellationToken ct)
    {
        ...
        {
            if (attrData.AttributeClass?.ToDisplayString() == "Vortex.EventDomainAttribute")
            {
        ...
        foreach (var member in classSymbol.GetMembers())
        {
                ...
                && field.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) == EventKeyFqn)
            {
                ...
                {
                    if (attr.AttributeClass?.ToDisplayString() == "Vortex.EventArgsAttribute"
    ...
    private static void Execute(SourceProductionContext spc, EventDomainInfo domain)
    {
        ...
        sb.AppendLine("// Generated by Vortex.Generators — do not edit.");
        ...
        {
            sb.AppendLine($"{ci}    [global::Vortex.EventArgs(typeof({evt.ArgsTypeFqn}))]");
        ...
        sb.AppendLine($"{ci}{fieldDecl} global::Vortex.EventManager<EventTypes> {managerField} = new({globalArg});");
        sb.AppendLine();
        sb.AppendLine($"{ci}/// <summary>Gets the <see cref=\"global::Vortex.EventManager{{T}}\"/> for this event domain. For instance domains, dispose this when the owner is no longer needed.</summary>");
        sb.AppendLine($"{ci}public{memberStatic} global::Vortex.EventManager<EventTypes> Manager => {managerField};");
        sb.AppendLine();
    ...
    private static void EmitUnitMethods(StringBuilder sb, string ci, string name, string memberStatic, string managerField)
    {
        ...
        sb.AppendLine($"{ci}    => global::Vortex.EventManager<EventTypes>.GlobalInvokeEvent(EventTypes.{name});");
        sb.AppendLine();
    ...
    private static void EmitTypedMethods(StringBuilder sb, string ci, string name, string argsType, string memberStatic, string managerField)
    {
        ...
        sb.AppendLine($"{ci}    => global::Vortex.EventManager<EventTypes>.GlobalInvokeEvent(EventTypes.{name}, args);");
        sb.AppendLine();
